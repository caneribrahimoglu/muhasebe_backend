...
class CRUDProduct(CRUDBase[Product, ProductCreate, ProductUpdate]):
    def create(self, db: Session, obj_in: ProductCreate):
        ...
        for comp in compatibilities_data:
            db.add(ProductCompatibility(
                ...
                model_codes=comp.model_codes,
                search_text=f"{comp.brand_name} {comp.model_codes}"
            ))
    ...
    def update(self, db: Session, db_obj: Product, obj_in: ProductUpdate):
        ...
        # Uyumluluk listesi varsa, önce mevcut olanları sil
        if comps is not None:
            ...
            for comp in comps:
                db.add(ProductCompatibility(
                    ...
                    model_codes=comp.get("model_codes"),
                    search_text=f"{comp.get('brand_name')} {comp.get('model_codes')}"
                ))

        # Ana ürün alanlarını güncelle
